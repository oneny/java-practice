public class MainChallenge {
  /**
   *
   * Modify the program so taht adding items to the shopping basket doesn't
   ...
   *
   * Items can continue to be added to the basket, but i should not be possible to
   * reserve more than the available stock of any item. An item's available stock
   * is the stock count less the reserved amount.
   *
   * The stock count for each items is reduced when a basket is check out, at which
   * point all reserved items in the basket have their actual stock count reduced.
   *
   * One checkout is complete, the contents of the basket are cleared.
   *
